...
public class JwtService {
    ...
    public Boolean validateToken(String token, UserDetails userDetails) {
        ...
        Token retrivedToken = tokenrepositery.findByTokenstring(token);//.map(t -> t.isIsloggedout().orElse(false));
        boolean checkingValidToken;
        if (retrivedToken.isIsloggedout()==false) {
        	checkingValidToken=true;
        }else {
        	checkingValidToken=false;	
        }
        return (username.equals(userDetails.getUsername()) && !isTokenExpired(token)&&checkingValidToken);
    }
